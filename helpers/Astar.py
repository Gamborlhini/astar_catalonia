...
def Astar(graph, start, end):
    ...
    # main loop of Astar, the "lets make a deal section"
    while any(v not in Found and dd[v] < float("inf") for v in dd):
        ...
        # iterates through all, and will update distance using the reduced h heuristic
        for eid in edge_ids:
            edge = graph.es[eid]
            ...
            if w not in Found:
                ...
                if true_cost[v] + h_reduced_length(edge,v_vertex,w_vertex,end_vertex) < true_cost[w]:
                    ...
                    predicesor[w] = v
    # if there are no more deals that can be offered, the network is exhausted and there is no path from s to t
    print("NO PATH WAS FOUND")



# reducing the travel time by the minimum distance travelled at the fastest speed
def h_reduced_length(edge, a, b, t) :
    return (edge["weight"]) - h(a,t)/120 + h(b,t)/120

# currently unused euclidian distance function
def euclidianDist(v, t):
    return ((v["coord"][0]-t["coord"][0])** 2 + (v["coord"][1]-t["coord"][1]) ** 2) ** .5

# actually used haversine distance function to convert lat, lon to km
def h(v, t):
    lon1 = v["coord"][0]
    lat1 = v["coord"][1]
    lon2 = t["coord"][0]
...
# creates a graph from the provided nodes and edges files generated by generate_csv.py
...
# Prints out the final path from node to node
...
# These are helper functions to parse the speed limit that is provided as OSM tags
...
# Sum the speed limits along the path
...
# Sum the total distance along the path
...
# Backtracks the path to find the true shortest path
